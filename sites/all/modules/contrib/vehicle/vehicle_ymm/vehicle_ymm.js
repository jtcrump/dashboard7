  ...
  Drupal.behaviors.vehicle_ymm_dropdown = {
    attach: function() {
      $('select.vehicle-ymm-year').change(function(){
        ...
        var path = Drupal.settings.basePath + 'vehicle/ymm/' + limitSelectorElement.val() + '/' + yearElement.val() + '/make';
        $.getJSON(path, function(j){
          ...
          for(var i = 0; i < optionsArray.length; i++) {
            ...
            if(thisOption[0].toLowerCase() == 'make') {
              options = '<option value="0">Make</option>'+options;
            } else {
              options += '<option value="' + thisOption[1] + '">' + thisOption[0] + '</option>';
            }
          }

          makeElement.html(options);
          modelElement.html('<option value="0">Model</option>');
      ...
      $('select.vehicle-ymm-make').change(function(){
        ...
        var trimElement = fieldsetElement.children('select.vehicle-ymm-trim');
        var limitSelectorElement = fieldsetElement.children('.vehicle-ymm-limit-selector');
        var path = Drupal.settings.basePath + 'vehicle/ymm/' + limitSelectorElement.val() + '/' + yearElement.val() + '/' + makeElement.val() + '/model';
        $.getJSON(path, function(j){
          var options = '';
          var optionsArray = [];
          $.each(j, function(key,val) { optionsArray.push(val+'.'+key) });
      ...
      // Sorting functions to solve for asynchronous nature of JSON parsing.
    }
  }
})(jQuery);
